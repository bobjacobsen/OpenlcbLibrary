...
final class OpenlcbLibraryTests: XCTestCase {
    ...
    func testOperation() {
        ...
        XCTAssertEqual(canPhysicalLayer.receivedFrames.count, 10) // should be 10 with the Initialization Complete, Verify Global and clock query
        ...
        XCTAssertEqual(canPhysicalLayer.receivedFrames[9].data, [1,1,0,0,1,0,0xF0,0x00]) // carries clock query
        ...
        // TODO: the following two have zero source alias - see RemoteNodeProcessor
        XCTAssertEqual("\(String(format:"0x%08X", canPhysicalLayer.receivedFrames[1].header))", "0x19828000") // PIP reqeust
        XCTAssertEqual("\(String(format:"0x%08X", canPhysicalLayer.receivedFrames[2].header))", "0x19DE8000") // SNIP request
